...
// Retuns the name of all directories (not recursively) inside a directory
#[tauri::command]
pub fn read_contents_of(mut path: String, variant: String) -> Vec<DirectoryContent> {
  path += "\\";

  match fs::read_dir(&path) {
    Ok(entries) => {
      let needed_dir_entries = entries
        .filter_map(|entry| { entry.ok() }) // keep valid Ok()s
        .filter(|entry| if variant == "folder" { entry.path().is_dir() } else { true }); // Throw away file when variant is "folder"
